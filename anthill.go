...
//Imeplement the flag.Value interface
...
func main() {
	...
	countMetricPtr := countCommand.String("metric", "chars", "Metric {chats|words|lines};.")
	...
	// Use flag.Var to create a flag of our own FlagType
	...
	/*
		Verify that a subcommand has been provicded
	...
	*/
	...
	if countCommand.Parsed() {
		...
		// If the metric is not substring, the substring flag must not be used
		if *countMetricPtr != "substring" && (*countSubstringPtr != "" || (&countStringList).String() != "[]") {
